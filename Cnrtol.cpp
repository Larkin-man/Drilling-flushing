...
//-----------------------------------------------------
void__fastcall TF1::FormCreatr(TObject*Sender)
{
lo=0;
}
...
if (pere<95)
{
txt=FloatToStrF(pere, ffGeneral,4,2);
...
{
na=(pere-94)*Mt/100; //Превышение над допустимым значением
...
if (Gkml<Gkmk)
{
perc=100*GW/Gkml;  //Процентное соотношение Gk м Gkмл
if (perc>=90)
{
if (pere<95)
{
...
{
na=(pere-94)*Gkmk/100;  //Первышение над допустиыи значением
txt=FloatToStrF(na,ffGeneral,4,2);
txt="Вес на крюке больше допустимого на "+txt+" тонн.";
WriteTxt(txt,clRed); //Вызов процедуры вывода нового сообщения
sndPlaySound("Error.WAV",SND_ASYNC); //Звуковой сигнал
...
ti=0;
before=0;
or(i=1;i<=n;i++) //Выявление значений из формулы вида Sum+Gki*ti'
{
if((all[i]=='+')&&(i>1))
{
for(j=1;<=(i-1);j++)
...
//-----------------------------------------------------
void __fastcall TFl::eTvrExit(TObject*Sender)
{
float Tvr, Tvrd, perc;
...
{
txt=FloatToStr((Tvr-Tvrd),ffGeneral,4,2);
...
if (Tpr<=Tprd)
{
perc= 100*Tpr/Tprd; //процентное соотношение Tpr и Tprd 
if (pere>=90)
if (pere<95)
{
...
//-----------------------------------------------------
void_fastcall TF1::Panel1Exit(TObject*Sender)
{
float kl, dn, dt, L, Vdr, Vk, perc, na;
...
//-----------------------------------------------------
void_fastcall TF1::eLcExit(TObject *Sender)
{
float L, Lс, d5, ch, na;
...
//-----------------------------------------------------
void_fastcall TF1::Pancl2Exit(TObject*Sender)
{
...
if (Pspo<Per)
{
perс= 100*Pspo/Pgr; //процентное соотношение Pspo и Pgr
if (perс>=90)
if (perс<95)
{
txt= FloatToStrF (perc,ffGeneral,4,2);
...
{
na= (perc-94)*Pgr/l00; //превышение над допустимым значением
txt= FloatToStrF(na,ffGeneral,4,2);
