...
{
	 Cd = 1; // Стоимость долота
 C = 1; //Стоимсоть 1 часа бурения (механического разрушения породы)
 Cep = 1; // Стоимость 1 часа СПО
 Cpr = 1; // Стоимость 1 часа промывки скважины
  T = 1; // Время бурения
 Tcp = 1; // Время СПО
 Tpr = 1;  // Время промывки
 H = 1; // Проходка
}
//---------------------------------------------------------------------------
...
Y[i] = NY[i];
if(i==1)
	Form1->LabeledEdit9->Text = FloatToStr(((Y[i] * 1000000000.0 + 0.5) / 1000000000.0));
else
	Form4->debug->Text = FloatToStr(Int(Y[i] * 1000000000.0 + 0.5) / 1000000000.0);
}
